...
进入阶段 5 前，先想清楚一件事
阶段 5（T20-T22.6）一共 7 个子任务、估算 ~18-22 小时，是整个项目第二大的阶段。建议不要按顺序闷头做，先排个优先级：
MUST（核心反馈闭环）：

T20 同类书计数提示（2h）— 上线后立刻有"哇这是第 23 本"的成就感
T22.5 BookDetailModal（3h）— 后面所有点击都依赖这个基建，先做

SHOULD（让 Dashboard 成立）：

T22 洞察看板主结构（5h）— 但可以先做 4 个 widget：环形图、Top 3、最近入库、今年趋势
T21 同类书推荐（2h）

NICE（视觉惊喜）：

T22 剩余 2 个 widget：作者国家分布、30 天热力图
T22.6 热词云（3-4h）— 这个最好看，但工程量也最大，留到最后

推荐节奏：T20 → T22.5 → T22 (4 widget) → T21 → 中场休息看效果 → 决定后两个 widget 和词云做不做、要不要先发 v1。
